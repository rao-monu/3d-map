...
export const CesiumComponent: React.FunctionComponent<{
  ...
  const initializeCesium = React.useCallback(async () => {
    if (cesiumContainerRef.current && !cesiumViewer.current) {
      CesiumJs.Ion.defaultAccessToken = `${process.env.NEXT_CESIUM_TOKEN}`;
  ...
  const isZoomAbove15 = React.useCallback(() => {
    if (cesiumViewer.current) {
      ...
      const height =
        cesiumViewer.current.scene.globe.ellipsoid.cartesianToCartographic(
          cameraPosition
        ).height;
  ...
  const addPlaceLabels = React.useCallback(
    async (places: any[]) => {
      if (cesiumViewer.current) {
        ...
        try {
          const clampedCartesians =
            await cesiumViewer.current.scene.clampToHeightMostDetailed(
              cartesians
            );
          ...
          places.forEach((place, index) => {
            ...
            const labelPosition = CesiumJs.Cartesian3.fromRadians(
              ...
              CesiumJs.Cartographic.fromCartesian(clampedPosition).height +
                heightOffset
            );
            ...
            cesiumViewer.current!.entities.add({
              ...
              label: {
                ...
                distanceDisplayCondition: new CesiumJs.DistanceDisplayCondition(
                  0,
                  6000
                ),
